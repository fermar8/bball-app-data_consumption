...
resource "aws_cloudwatch_event_rule" "players_injuries_daily" {
  name                = "${var.players_injuries_function_name}-${var.environment}-daily"
  description         = "Daily trigger for players_injuries data consumption"
  schedule_expression = "cron(0 13 * * ? *)"
  state               = var.players_injuries_scheduler_enabled ? "ENABLED" : "DISABLED"

  tags = merge(var.tags, { Environment = var.environment })
}

resource "aws_cloudwatch_event_target" "players_injuries_daily_lambda" {
  rule      = aws_cloudwatch_event_rule.players_injuries_daily.name
  target_id = "${var.players_injuries_function_name}-lambda"
  arn       = aws_lambda_function.players_injuries_function.arn
}

resource "aws_lambda_permission" "allow_events_invoke_players_injuries" {
  statement_id  = "AllowExecutionFromCloudWatchEvents"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.players_injuries_function.function_name
  principal     = "events.amazonaws.com"
